...
def build_qdq_transpose_model(input_shape, model_path):
    """构建满足 DequantizeLinear -> Transpose -> QuantizeLinear 的测试模型。"""
    scale_value = 0.039
    zero_point = 0
    wrap_bias = 1e-3
    ...
    output_shape = [input_shape[0], input_shape[2], input_shape[1]]
    ...
    print(f"✓ 生成满足 DQ -> Transpose -> Q 的模型: {model_path}")
...
def main():
    ...
    build_qdq_transpose_model(input_shape, model_path)
    ...
    print("✓ 模型结构: input -> Add -> Q -> DQ -> Transpose -> Q -> DQ -> Sub -> output")

    model = onnx.load(model_path)
    if validate_dq_op_q_pattern(model, "Transpose"):
        print("✓ Pattern 检查通过: DequantizeLinear -> Transpose -> QuantizeLinear")
    else:
        raise RuntimeError("Pattern 检查失败: 未找到 DQ -> Transpose -> Q")
