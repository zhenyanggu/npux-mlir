...
class Vgg16LayerCoverageModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv = nn.Conv2d(3, 8, kernel_size=3, stride=1, padding=1, bias=True)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.fc = nn.Linear(8 * 16 * 16, 32, bias=True)
...
def ensure_unique_vgg16_layer_ops_exist(model_path):
    model = onnx.load(model_path)
    op_types = {node.op_type for node in model.graph.node}

    # Some exporters/quant flows rewrite semantically equivalent ops:
    # Flatten -> Reshape, Gemm -> MatMul(+Add).
    required_groups = {
        "Conv": {"Conv"},
        "Relu": {"Relu"},
        "MaxPool": {"MaxPool"},
    ...
    missing = [
        ...
        if op_types.isdisjoint(candidates)
    ]
    if missing:
        raise RuntimeError(
            "Missing required layer ops in vgg16_debug: "
            f"{missing}; present ops: {sorted(op_types)}"
        )


def main():
    model_name = "vgg16_debug"
    ...
    model = Vgg16LayerCoverageModel().eval()
    ...
    quantize_static(
        ...
        op_types_to_quantize=["Conv", "Relu", "MaxPool", "Gemm"],
    ...
    ensure_unique_vgg16_layer_ops_exist(model_path)
    ...
    print("Layer coverage (dedup): Conv, Relu, MaxPool, Flatten, Gemm")
