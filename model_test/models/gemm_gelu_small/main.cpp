...
VerificationSummary compareOutputs(const float *actual,
    const std::vector<float> &golden, int64_t count, float threshold = 0.2f) {
  struct DiffItem {
    int64_t index;
    float actual;
    float golden;
  ...
  std::cout << "Error Count (错误点数量/总点数量): " << errorCount << "/"
            << count << std::endl;
...
int main(int argc, char **argv) {
  ...
  const fs::path exeDir =
      exePath.has_parent_path() ? exePath.parent_path() : fs::path(".");
  const std::string modelPrefix = modelPrefixFromExecutable(exePath);

  const std::string inputFile =
      resolveDataFile(exeDir, modelPrefix, "input.bin");
  ...
  float *outputData =
      reinterpret_cast<float *>(omTensorGetDataPtr(outputTensor));
  ...
  std::vector<float> goldenData =
      loadBinaryFloatFile(goldenFile, outputElements);
  VerificationSummary summary =
      compareOutputs(outputData, goldenData, outputElements);
