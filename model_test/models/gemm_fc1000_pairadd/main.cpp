...
constexpr int64_t kInputFeatures = 4096;
constexpr int64_t kOutputFeatures = 1000;
...
void verifyShape(const int64_t *shape, int64_t rank) {
  const bool ok = rank == 2 && shape[0] == kBatch && shape[1] == kOutputFeatures;
...
int main(int argc, char **argv) {
  ...
  const auto input = loadFloatBin(inputFile, kBatch * kInputFeatures);
  const auto golden = loadFloatBin(goldenFile, kBatch * kOutputFeatures);

  int64_t inputShape[] = {kBatch, kInputFeatures};
  OMTensor *inputTensor = omTensorCreate(
      const_cast<float *>(input.data()), inputShape, 2, ONNX_TYPE_FLOAT);
  ...
  const auto summary =
      compareOutputs(output, golden, kBatch * kOutputFeatures, envToFloat("MODEL_TEST_THRESHOLD", 0.1f));
