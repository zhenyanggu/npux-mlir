...
MODEL_NAME = "bert_base_layer_matmul_probv_1x12x128x128_to_1x12x128x64"
INPUT_SHAPE = (1, 12, 128, 128)
RHS_SHAPE = (1, 12, 128, 64)
...
def build_qdq_matmul_model(model_path, seed=2026):
    rng = np.random.default_rng(seed)
    wrap_bias = 1e-3

    weight_fp32 = rng.standard_normal(size=RHS_SHAPE, dtype=np.float32)
    ...
    initializers = [
        ...
        helper.make_tensor("weight_q", TensorProto.INT8, list(RHS_SHAPE), weight_q.reshape(-1).tolist()),
    ]

    nodes = [
        helper.make_node("Add", ["input", "wrap_add"], ["wrapped_input"], name="node_add"),
        helper.make_node(
            "QuantizeLinear", ["wrapped_input", "input_scale", "zero_point"], ["input_q"],
            name="quantize_before_matmul",
        ),
        helper.make_node(
            "DequantizeLinear", ["input_q", "input_scale", "zero_point"], ["input_dq"],
            name="dequant_before_matmul_input",
        ),
        helper.make_node(
            "DequantizeLinear", ["weight_q", "weight_scale", "zero_point"], ["weight_dq"],
            name="dequant_before_matmul_weight",
        ),
        helper.make_node("MatMul", ["input_dq", "weight_dq"], ["matmul_fp32"], name="node_matmul"),
        helper.make_node(
            "QuantizeLinear", ["matmul_fp32", "output_scale", "zero_point"], ["matmul_q"],
            name="quantize_after_matmul",
        ),
        helper.make_node(
            "DequantizeLinear", ["matmul_q", "output_scale", "zero_point"], ["dequantized_output"],
    ...
    graph = helper.make_graph(
        ...
        [helper.make_tensor_value_info("output", TensorProto.FLOAT, list(calib_output.shape))],
        initializers,
    )
...
def main():
    ...
    np.random.seed(2026)
    build_qdq_matmul_model(model_path)
    ...
    print(f"Case: {MODEL_NAME}, input shape: {INPUT_SHAPE}, rhs shape: {RHS_SHAPE}")
