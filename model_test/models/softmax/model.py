...
class WrappedSoftmaxModel(nn.Module):
    def __init__(self):
        super().__init__()
...
def main():
    ...
    # 贴近 Transformer 常见维度: [batch, seq, hidden]
    input_shape = (1, 128, 768)

    torch.manual_seed(2026)
    np.random.seed(2026)

    model = WrappedSoftmaxModel().eval()
    ...
    reader = RandomDataReader(input_name="input", input_shape=input_shape, num_batches=10)
    quantize_static(
    ...
    artifacts = [model_path, input_path, golden_path]
    ...
    print("Quantization: Softmax-only, INT8 symmetric (Activation/Weight symmetric)")
