...
def _ensure_input_model(input_model: Path) -> None:
    ...
    if cache_model.exists():
        print(f"[copy] cached ResNet50 model: {cache_model} -> {input_model}")
...
def _check_model_io_shape(session: Any) -> None:
    ...
    if len(inputs) != 1:
        raise RuntimeError(f"Expected single input for ResNet50, got {len(inputs)}")
    if len(outputs) < 1:
...
def run_qdq_quantization(
    ...
    reader = ResnetCalibrationDataReader(
...
def generate_artifacts(
    ...
    if sampling_mode not in {"contiguous", "balanced"}:
        raise ValueError(f"Unsupported sampling mode: {sampling_mode}")
    ordered = list(all_samples) if sampling_mode == "contiguous" else _interleave_items_by_label(all_samples)
    if sample_offset >= len(ordered):
        raise ValueError(f"sample_offset={sample_offset} out of range, total={len(ordered)}")
    candidates = ordered[sample_offset:]
    ...
    for source_index, (image_path, label) in enumerate(candidates, start=sample_offset):
        rgb = _load_image_rgb(image_path)
        resized = _resize_shorter_side(rgb, RESIZE_SHORTER)
        cropped = _center_crop(resized, IMAGE_SIZE, IMAGE_SIZE)
        ...
        if len(images_chw_u8) >= sample_count:
            break

    if len(images_chw_u8) == 0:
        raise RuntimeError(
            "No samples with golden_pred==label were found from current sampling range. "
            "Try reducing --sample-offset or use a different --image-list."
        )
    if len(images_chw_u8) < sample_count:
        raise RuntimeError(
            f"Insufficient valid samples: requested {sample_count}, got {len(images_chw_u8)}. "
            f"Scanned={scanned}, filtered_out(label!=golden_pred)={skipped_label_mismatch}. "
            "Try reducing --sample-count or --sample-offset."
        )
    ...
    print("Generated ResNet50 test artifacts:")
    ...
    print(
        f"Filtered samples by golden correctness: kept={len(images_np)}, "
        f"filtered_out(label!=golden_pred)={skipped_label_mismatch}, scanned={scanned}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate ResNet50 test data from local ImageNet subset without full dataset download"
    )
    parser.add_argument("--input-model", default="resnet50-v1-12.onnx")
    ...
    parser.add_argument(
        ...
        help="Directory to export preview PNG images (default: resnet50_preview_png)",
    )
    parser.add_argument(
