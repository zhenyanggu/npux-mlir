...
def build_qdq_maxpool_model(input_shape, model_path):
    """构建满足 DequantizeLinear -> MaxPool -> QuantizeLinear 的测试模型。"""
    scale_value = 0.039
    zero_point = 0
    wrap_bias = 1e-3
    ...
    nodes = [
        helper.make_node(
            "Add",
            inputs=["input", "wrap_add"],
            outputs=["wrapped_input"],
            name="node_add",
        ),
        helper.make_node(
            "QuantizeLinear",
            inputs=["wrapped_input", "scale", "zero_point"],
            outputs=["input_q"],
        ...
        helper.make_node(
            "QuantizeLinear",
            inputs=["maxpool_fp32", "scale", "zero_point"],
            outputs=["maxpool_q"],
            name="quantize_after_maxpool",
        ),
        helper.make_node(
            "DequantizeLinear",
            inputs=["maxpool_q", "scale", "zero_point"],
            outputs=["dequantized_output"],
            name="dequant_after_maxpool",
        ),
        helper.make_node(
            "Add",
            inputs=["dequantized_output", "wrap_sub"],
            outputs=["output"],
            name="node_sub",
        ),
    ]
    ...
    if not validate_dq_op_q_pattern(reloaded, "MaxPool"):
        raise RuntimeError("Generated MaxPool model does not satisfy DQ -> MaxPool -> Q pattern")

    print(f"✓ 生成满足 DQ -> MaxPool -> Q 的模型: {model_path}")
...
def main():
    ...
    build_qdq_maxpool_model(input_shape, model_path)
    ...
    print("✓ 模型结构: input -> Add -> Q -> DQ -> MaxPool -> Q -> DQ -> Sub -> output")

    model = onnx.load(model_path)
    if validate_dq_op_q_pattern(model, "MaxPool"):
        print("✓ Pattern 检查通过: DequantizeLinear -> MaxPool -> QuantizeLinear")
    else:
        raise RuntimeError("Pattern 检查失败: 未找到 DQ -> MaxPool -> Q")
