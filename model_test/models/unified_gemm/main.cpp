...
/* Hold verification statistics for the final result line. */
...
/* Keep one diff entry so the report can print the largest errors. */
struct ErrorItem {
  int64_t index;
  float actual;
  float golden;
...
/* Parse one integer environment variable used by the GEMM verifier. */
...
/* Parse one floating-point environment variable used by the GEMM verifier. */
...
/* Check whether the executable name carries the board suffix. */
...
/* Derive the model prefix from the executable file name. */
...
/* Resolve a model-specific file name and keep the legacy fallback. */
...
/* Read one float32 binary file and verify its element count. */
...
/* Compare runtime output with golden data and optionally summarize the error layout. */
VerificationSummary compareOutputs(
    const float *actual, const std::vector<float> &golden, int64_t count, int64_t rows, int64_t cols,
    float threshold = 0.1f, int64_t topK = 10, bool printLayoutStats = true) {
  ...
  std::cout << "Error Count (错误点数量/总点数量): " << errorCount << "/" << count << std::endl;
  ...
  if (printLayoutStats && errorCount > 0 && rows > 0 && cols > 0) {
    ...
    for (int64_t i = 0; i < std::min<int64_t>(5, static_cast<int64_t>(rowRank.size())); ++i) {
      if (rowRank[static_cast<size_t>(i)].second == 0)
        break;
      std::cout << " r" << rowRank[static_cast<size_t>(i)].first << ":" << rowRank[static_cast<size_t>(i)].second;
    }
    std::cout << std::endl;
    ...
    for (int64_t i = 0; i < std::min<int64_t>(8, static_cast<int64_t>(blockRank.size())); ++i) {
      if (blockRank[static_cast<size_t>(i)].second == 0)
        break;
      std::cout << " b" << blockRank[static_cast<size_t>(i)].first << ":" << blockRank[static_cast<size_t>(i)].second;
    }
    std::cout << std::endl;

    std::cout << "[Error Layout] Top lanes (lane=col%32):";
    for (int64_t i = 0; i < std::min<int64_t>(8, static_cast<int64_t>(laneRank.size())); ++i) {
      if (laneRank[static_cast<size_t>(i)].second == 0)
        break;
      std::cout << " l" << laneRank[static_cast<size_t>(i)].first << ":" << laneRank[static_cast<size_t>(i)].second;
    }
    std::cout << std::endl;
  }
...
/* Print the final parser-friendly summary line required by model_test. */
void printFinalSummary(const VerificationSummary &summary) {
  std::cout << "@@MODEL_TEST_RESULT@@ errors=" << summary.errorCount << "/"
            << summary.totalCount << " status=" << (summary.passed ? "PASS" : "FAIL")
            << std::endl;
}

/* Validate the expected output shape of the unified GEMM testcase. */
...
int main(int argc, char **argv) {
  ...
  std::cout << "[Verify Config] threshold=" << threshold
            << ", topK=" << topK
            << ", layoutStats=" << (printLayoutStats ? "on" : "off") << std::endl;
  ...
  const VerificationSummary summary =
      compareOutputs(outputData, golden, outputElements, rows, cols, threshold, topK, printLayoutStats);
