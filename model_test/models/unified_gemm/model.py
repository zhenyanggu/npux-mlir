...
class WrappedGemmModel(nn.Module):
    """Build the unified GEMM testcase with the same structure as the original GEMM case."""

    def __init__(self, in_features=768, out_features=1024, use_bias=True):
        """Create the Linear layer under test with the original GEMM semantics."""
        super().__init__()
        self.fc = nn.Linear(in_features, out_features, bias=use_bias)

    def forward(self, x):
        """Wrap GEMM with tiny offsets so the target node stays easy to identify."""
        x = x + 1e-3
        x = self.fc(x)
        x = x - 1e-3
        return x
...
def parse_args():
    """Parse generator arguments with the same interface as the original GEMM case."""
    parser = argparse.ArgumentParser(description="Generate unified GEMM quant model/input/golden")
...
def main():
    """Export, quantize, and serialize the unified GEMM testcase artifacts."""
    args = parse_args()

    model_name = "unified_gemm"
