...
def _check_model_io_shape(session: Any) -> None:
    ...
    if len(inputs) != 1:
        raise RuntimeError(f"Expected single input for {MODEL_NAME}, got {len(inputs)}")
    if len(outputs) < 1:
...
def run_qdq_quantization(
    ...
    quantize_static(
        ...
        op_types_to_quantize=["MatMul", "LayerNorm", "Softmax", "Gelu", "Gemm", "Transpose", "MaxPool", "Relu"],
        extra_options={
...
def generate_artifacts(
    ...
    print(f"Generated {MODEL_NAME} test artifacts:")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=f"Generate {MODEL_NAME} test data from local ImageNet subset without full dataset download"
    )
    parser.add_argument("--input-model", default="mobilenetv2-12.onnx")
    ...
    parser.add_argument(
        ...
        help=f"Directory to export preview PNG images (default: {MODEL_NAME}_preview_png)",
    )
    parser.add_argument(
