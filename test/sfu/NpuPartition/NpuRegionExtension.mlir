...
module attributes {llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "onnx-mlir.symbol-postfix" = "act_test_quant_sym"} {
  func.func @main_graph(%arg0: tensor<1x32xf32> {onnx.name = "input"}) -> (tensor<1x32xf32> {onnx.name = "output"}) {
    ...
    %6 = scf.execute_region -> tensor<1x32xf32> {
      %8 = "onnx.QuantizeLinear"(%5, %3, %2) {axis = 1 : si64, onnx_node_name = "add_QuantizeLinear", saturate = 1 : si64} : (tensor<1x32xf32>, tensor<f32>, tensor<i8>) -> tensor<1x32xi8>
      %9 = bufferization.alloc_tensor() : tensor<1x32xi8>
      %10 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"], library_call = "npu_layernorm"} ins(%8 : tensor<1x32xi8>) outs(%9 : tensor<1x32xi8>) attrs =  {axis = -1 : i64, epsilon = 9.99999974E-6 : f32, in_scale = 0.0227836408 : f32, in_zp = 0 : i32, npu.target = "npu", out_scale = 0.0231352877 : f32, out_zp = 0 : i32} {
      ^bb0(%in: i8, %out: i8):
        %16 = arith.addi %in, %in : i8
        linalg.yield %16 : i8
      } -> tensor<1x32xi8>
      %11 = bufferization.alloc_tensor() : tensor<1x32xi8>
      %12 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"], library_call = "npu_gelu"} ins(%10 : tensor<1x32xi8>) outs(%11 : tensor<1x32xi8>) attrs =  {in_scale = 0.0231352877 : f32, in_zp = 0 : i32, npu.target = "npu", out_scale = 0.0230970979 : f32, out_zp = 0 : i16} {
      ^bb0(%in: i8, %out: i8):
        %16 = arith.addi %in, %in : i8
        linalg.yield %16 : i8
      } -> tensor<1x32xi8>
      %13 = bufferization.alloc_tensor() : tensor<1x32xi8>
      %14 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"], library_call = "npu_softmax"} ins(%12 : tensor<1x32xi8>) outs(%13 : tensor<1x32xi8>) attrs =  {axis = -1 : i64, in_scale = 0.0230970979 : f32, in_zp = 0 : i32, npu.target = "npu", out_scale = 0.00787401571 : f32, out_zp = 0 : i16} {
      ^bb0(%in: i8, %out: i8):
        %16 = arith.addi %in, %in : i8
        linalg.yield %16 : i8
      } -> tensor<1x32xi8>
      %15 = "onnx.DequantizeLinear"(%14, %4, %2) {axis = 1 : si64, onnx_node_name = "softmax_DequantizeLinear"} : (tensor<1x32xi8>, tensor<f32>, tensor<i8>) -> tensor<1x32xf32>
      scf.yield %15 : tensor<1x32xf32>
    } {npu.region_extended}
