...
struct ConvConfig {
    ...
    uint8_t is_bias;            // 0=accumulate psum, 1=accumulate bias
...
struct GemmConfig {
    // config_compute
    uint8_t  dataflow;            // 0=im2col & OS, 1=OS only
    uint8_t  int_type;            // 00 for int8
    uint8_t  optype;              // 1=Conv (0=GEMM, 2=GEMV)
    uint8_t  accout_dest;         // 0=SPM, 1=ACC
    uint16_t input_a_zeropoint;
    uint16_t input_b_zeropoint;

    uint32_t output_zeropoint;    // ACC to SPM zeropoint
    uint16_t output_scale;        // ACC to SPM scale
    ...
    uint32_t output_addr;         // Output matrix address (To ACC/SPM)
    uint16_t output_stride;       // Output matrix stride  (To ACC/SPM)
    uint8_t  isaccu;              // 0=No accumulate, 1=Accumulate with previous psum
    uint8_t  relu;                // 0=Disabled, 1=Enabled
    uint8_t  relu_type;           // 0=relu, 1=relu6, 2=leaky(0.1), 3=leaky(0.2), 4=leaky(0.01)
    uint8_t  is_bias;             // 0=accumulate psum, 1=accumulate bias
