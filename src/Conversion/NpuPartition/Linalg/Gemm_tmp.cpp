...
static Value createGenericMatMulOp(
  ...
  auto executeRegion = rewriter.create<scf::ExecuteRegionOp>(loc, outType);
  {
    ...
    // Map 构建 Helper：按输入实际 rank 对齐 batch 维，避免 A/B batch 维不对齐
    // 迭代维顺序: [batch..., m, n, k]
    auto getMap = [&](bool isA, bool isB, bool isC, Value val = Value()) -> AffineMap {
      ...
      auto appendBatchDimsForOperand = [&](Value operand) {
        int64_t outBatchRank = outRank - 2;
        int64_t opRank = mlir::cast<RankedTensorType>(operand.getType()).getRank();
        int64_t opBatchRank = std::max<int64_t>(0, opRank - 2);
        int64_t batchStart = outBatchRank - opBatchRank;
        for (int64_t i = 0; i < opBatchRank; ++i)
          exprs.push_back(rewriter.getAffineDimExpr(batchStart + i));
      };

      if (isA) {
        appendBatchDimsForOperand(val);
        exprs.push_back(m);
        exprs.push_back(k);
      } else if (isB) {
        appendBatchDimsForOperand(val);
    ...
    gemmIndexingMaps.push_back(getMap(true, false, false, inputs[0]));  // A
    gemmIndexingMaps.push_back(getMap(false, true, false, inputs[1]));  // B
