//=======================================================
//src/Conversion/NpuToLLVM/CustomBufferLoopHoist.cpp
// Custom Buffer Loop Hoisting for not hoisting NPU loops
//=======================================================
...
static bool defaultIsSmallAlloc(Value alloc, unsigned maximumSizeInBytes,
  ...
  if (!type.hasStaticShape()) {
    ...
    if (type.getRank() <= maxRankOfAllocatedMemRef) {
      return llvm::all_of(
          alloc.getDefiningOp()->getOperands(), [&](Value operand) {
...
static bool leavesAllocationScope(
    Region *parentRegion, const BufferViewFlowAnalysis::ValueSetT &aliases) {
...
static bool hasAllocationScope(
    Value alloc, const BufferViewFlowAnalysis &aliasAnalysis) {
...
struct CustomBufferAllocationLoopHoistingState
    : BufferAllocationHoistingStateBase {
  using BufferAllocationHoistingStateBase::BufferAllocationHoistingStateBase;
  ...
  bool isLegalPlacement(Operation *op) {
    bool isLegal = isSequentialLoop(op) &&
...
struct CustomBufferLoopHoistingPass
    : public PassWrapper<CustomBufferLoopHoistingPass,
          OperationPass<func::FuncOp>> {

  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CustomBufferLoopHoistingPass)

  StringRef getArgument() const override {
    return "custom-buffer-loop-hoisting";
  }
  StringRef getDescription() const override {
    return "Hoist buffer allocations from loops (custom filter).";
  }

  void runOnOperation() override {
    // 直接在这里实例化，不需要外部函数
    BufferAllocationHoisting<CustomBufferAllocationLoopHoistingState> optimizer(
        getOperation());
    optimizer.hoist();
  }
};
...
std::unique_ptr<Pass> npux::createCustomBufferLoopHoistingPass() {
  return std::make_unique<CustomBufferLoopHoistingPass>();
}
