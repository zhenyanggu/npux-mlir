...
llvm::Error VersaPBankScheduler::validateTile(
  ...
  if (request.needsAccumulate)
    return invalid("ACC accumulation requires a prior write_partial SA command, which is outside the single-tile scheduler slice");
  if (request.needsVpu)
    return invalid("VPU is outside the G1 single-GEMM scheduler slice");
  if (request.operation == SaOperation::AttentionQk) {
    if (outputMode != OutputMode::RawInt32 || request.needsAccumulate ||
        request.needsResadd || !request.outputDma || !request.outputDma->qkMode)
  ...
  if (request.operation == SaOperation::AttentionPv &&
      (outputMode != OutputMode::TensorInt8 || request.needsAccumulate ||
       request.needsBias || request.outputDma && request.outputDma->qkMode))
  ...
  if (request.outputDma) {
    const uint32_t rowBytes = outputMode == OutputMode::TensorInt8 ? request.n
        : outputMode == OutputMode::Bf16 ? static_cast<uint32_t>(request.n) * 2
        : static_cast<uint32_t>(request.n) * 4;
    if (rowBytes > std::numeric_limits<uint16_t>::max() ||
        request.outputDma->rowCount != request.m ||
        request.outputDma->rowBytes != rowBytes)
      return invalid("MVOUT shape must match the GEMM output mode and tile");
  }
...
uint32_t VersaPBankScheduler::appendCommand(ScheduledCommandKind kind,
  ...
  ScheduledCommand command{nextCommandId++, kind, engine, descriptor,
      ...
      std::move(reads),
      std::move(writes), {}};
  addDependency(command.dependencies, engineLastUse[engineIndex(engine)]);
  ...
  engineLastUse[engineIndex(engine)] = command.id;
  for (const BankUse &use : command.reads)
...
llvm::Expected<TileSchedule> VersaPBankScheduler::scheduleGemmTile(
  ...
  if (request.needsAccumulate)
    result.accumulatorBank = chooseBank(BankResource::O, {result.oBank});
  ...
  const bool usesMetadata = request.needsBias ||
                            request.quantization.metadataWords != 0 ||
                            scaleMode == ScaleMode::PerChannel;
  result.accumulatorIsChange = request.needsAccumulate;
  ...
  if (request.needsAccumulate)
    gemmReads.push_back({BankResource::O, result.accumulatorBank});
  if (request.needsResadd)
    gemmReads.push_back({BankResource::Resadd, result.residualBank});
  appendCommand(ScheduledCommandKind::Gemm, EngineKind::Sa, *gemmDescriptor,
